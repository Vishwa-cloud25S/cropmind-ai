...
> **Status: Phases 0–7 done — backend core live (14-table schema, uploads pipeline, DB-backed
> analysis queue + worker, `/images` `/analyses` `/predictions` `/farms` `/fields` `/intervention-zones`),
> the real frontend (dashboard, analyze wizard, analysis view with Grad-CAM, history, farms/fields CRUD,
> model information) and the field map (Leaflet/OSM boundaries, evidence-space zones, human review,
> simulation-labelled exports); formal M1 gates measured 2026-08-09 (Gate C, operator CPU).**
...
- Web app → http://localhost:3000 — real UI: `/dashboard`, `/analyze` (upload → analysis wizard),
  `/analyses` (history), `/farms`, `/map` (boundaries, zones, review + exports), `/model-information`
...
| [14 — Development roadmap](docs/14-roadmap.md) | ✅ Phase 0, updated through Phase 7 |
